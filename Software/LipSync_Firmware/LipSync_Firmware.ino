...
_cursor setting1 = {5, -1.4, default_cursor_speed - (4 * delta_cursor_speed)}; // 5,-1.0,10
_cursor setting2 = {5, -1.4, default_cursor_speed - (3 * delta_cursor_speed)}; // 5,-1.2,10
_cursor setting3 = {5, -1.4, default_cursor_speed - (2 * delta_cursor_speed)};
_cursor setting4 = {5, -1.4, default_cursor_speed - (delta_cursor_speed)};
_cursor setting5 = {5, -1.4, default_cursor_speed};
_cursor setting6 = {5, -1.4, default_cursor_speed + (delta_cursor_speed)};
_cursor setting7 = {5, -1.4, default_cursor_speed + (2 * delta_cursor_speed)};
_cursor setting8 = {5, -1.4, default_cursor_speed + (3 * delta_cursor_speed)};
_cursor setting9 = {5, -1.4, default_cursor_speed + (4 * delta_cursor_speed)};
...
void loop() {
  ...
  if (single == 0) {
    Serial.println(" ");
    Serial.println(" --- ");
    Serial.println("This is the 28 January - Stable Fast Start-Up FSR version [manual + auto calibration].");
    Serial.println(" ");
    Serial.println("Enhanced functions:");
    Serial.println("Tap and drag");
    Serial.println("Scrolling");
    Serial.println("Joystick calibration");
    Serial.println("Automatic home position calibration");
    Serial.println(" --- ");
  ...
  if (xh > x_box_right || xl > x_box_left || yh > y_box_up || yl > y_box_down) {
    ...
    if (poll_counter >= 3) {
      ...
      if (comm_mode == 0) {
        ...
        if (xh > x_box_right && (yh < y_box_up && yl < y_box_down)) {
          mouseCommand(cursor_click_status, x_cursor_high(xh), 0, 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if (xl > x_box_left && (yh < y_box_up && yl < y_box_down)) {
          mouseCommand(cursor_click_status, x_cursor_low(xl), 0, 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if (yh > y_box_up && (xh < x_box_right && xl < x_box_left)) {
          mouseCommand(cursor_click_status, 0, y_cursor_high(yh), 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if (yl > y_box_down && (xh < x_box_right && xl < x_box_left)) {
          mouseCommand(cursor_click_status, 0, y_cursor_low(yl), 0);
          delay(cursor_delay);
          poll_counter = 0;
        } else if (xh > x_box_right && yh > y_box_up) {
  ...
  int y_cursor_high(int j) {
    ...
    int k = (int)(round(-1.0 * cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((yh_comp) * (float)(j - y_box_up)) / y_box_up))))));
  ...
  int y_cursor_low(int j) {
    ...
    int k = (int)(round(1.0 * cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((yl_comp) * (float)(j - y_box_down)) / y_box_down))))));
  ...
  int x_cursor_high(int j) {
    ...
    int k = (int)(round(1.0 * cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((xh_comp) * (float)(j - x_box_right)) / x_box_right))))));
  ...
  int x_cursor_low(int j) {
    ...
    int k = (int)(round(-1.0 * cursor_max_speed * (1.0 - pow(2.718, (cursor_factor * (((xl_comp) * (float)(j - x_box_left)) / x_box_left))))));
  ...
  void Joystick_Initialization(void) {
    ...
    box_delta = 25;                         // Threshold digital value for FSR joystick to exceed before cursor movement occurs
    ...
    // Change from box_delta = 30 to box_delta = 25 on 22 Jan 2017; revert as necessary
    // Emergent drift was occurring at
  ...
  void Joystick_Calibration(void) {
    ...
    int delta_max_total = (yh_max - y_box_up) + (yl_max - y_box_down) + (xh_max - x_box_right) + (xl_max - x_box_left);
    ...
    yh_comp = avg_delta_max / (yh_max - y_box_up);
    yl_comp = avg_delta_max / (yl_max - y_box_down);
    xh_comp = avg_delta_max / (xh_max - x_box_right);
    xl_comp = avg_delta_max / (xl_max - x_box_left);
  ...
  void Auto_Home_Calibration(void){
      ...
      //box_delta = 15;                         // Threshold digital value for FSR joystick to exceed before cursor movement occurs
                                            //***CAN BE CHANGED TO CREATE MORE SENSITIVE MOVEMENT ACTIVATION *ORIGINALLY 25
                                            // A large value tends to minimize frequency of emergent cursor drift
                                            // Change from box_delta = 30 to box_delta = 15 on 22 Jan 2017; revert as necessary
